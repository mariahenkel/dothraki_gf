...
Of course, it is not feasible either to implement the entire
...
David J.\ Peterson \cite{peterson-web},
the community-operated ``Tongues of Ice and Fire`` wiki
at \url{wiki.dothraki.org}. These tend to be great resources
...
has done great work organizing and detailing this information.
...
mentioned above (the two largest remaining omissions are wh-questions and
numerals). In many cases we have run up against the limits
...
\begin{itemize}
  ...
  When they have appeared on the show, verb phrase
  ...
  in the nominative. In the end, we had no other choice
  but to guess the declension (we decided on standard
  animate inflection).
  
  Similarly, ``eyak'' supposedly means ``everybody'', according
  to the vocabulary, but in the only place it is used
  in the corpus, it seems to be used in the sense of ``everything''.
